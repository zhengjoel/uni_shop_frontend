<template>
    <view class="coupon-item">
		<view class="coupon-money">
			...
			<view class="layof" :style="{color:theme}">￥{{item.value}}</view>
			<view v-if="!types">
				<view class="tit" style="display: none;">券号：</view>
				<view class="demand">{{item.title}}</view>
			</view>
			<view class="end_time">开始时间: {{item.starttime_text}}</view>
			<view class="end_time">结束时间: {{item.endtime_text}}</view>
		</view>
