	...
	<view class="container">
		<view class="carousel">
			<swiper indicator-dots circular=true duration="400">
				<swiper-item class="swiper-item" v-for="(item,index) in product.images.split(',')" :key="index">
...
<script>
	...
	export default{
		...
		computed:{
			...
			specProduct(){
				if(this.product.specStatus){
					...
					return {market_price,sales_price,stock,image};
				}else{
					return this.product;
				}
			}
		...
		async onLoad(options){
			let id = options.id;
			const that = this;
			let res = await that.$api.request('/product/info?id='+id,'GET');
			if(res.data.code){
